...
class Individual(Agent):
    ...
    def count_friends(self, population, G):
    ...
    def spread_beliefs(self, population, G):
        ...
        for y in neighbors:
            ...
            if population[y].belief == Thought.NEUTRAL:
                self.none += 1
        if self.belief == Thought.PRO or self.belief == Thought.NEUTRAL:
        ...
        if self.belief == Thought.ANTI:
            if self.pro_friend > self.anti_friend + self.none:
                self.belief = Thought.PRO
            elif self.none > self.anti_friend + self.pro_friend:
                self.belief = Thought.NEUTRAL
            else:
                self.belief = Thought.ANTI
